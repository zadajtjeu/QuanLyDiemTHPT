...
if((empty($_SESSION['username']) && empty($_SESSION['password']))) {
	...
	if ($result->num_rows > 0) {
		...
		if (in_array($taikhoan['role'], array('admin', 'manager'))) {
			...
			$columnName =  'maNH'; // Column name
			if (is_array($_POST['columns'][$columnIndex]['data'])) {
				$columnName = empty($_POST['columns'][$columnIndex]['data']['sort']) ? 'maNH' : htmlspecialchars($_POST['columns'][$columnIndex]['data']['sort']);
			} else {
				$columnName = empty($_POST['columns'][$columnIndex]['data']) ? 'maNH' : htmlspecialchars($_POST['columns'][$columnIndex]['data']);
			}
			...
			$searchQuery = " ";
			if($searchValue != ''){
			   $searchQuery .= " AND (`namHoc` like '%".$searchValue."%') ";
			}
			...
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `namhoc`");
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecords = $records['allcount'];
			 
			## Total number of records with filtering
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `namhoc` WHERE 1 ".$searchQuery);
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecordwithFilter = $records['allcount'];
			 
			## Fetch records
			$empQuery = "SELECT * FROM `namhoc` WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
			//echo $empQuery;
