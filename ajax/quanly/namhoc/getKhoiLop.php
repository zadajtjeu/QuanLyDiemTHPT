...
if((empty($_SESSION['username']) && empty($_SESSION['password']))) {
	...
	if ($result->num_rows > 0) {
		...
		if (in_array($taikhoan['role'], array('admin', 'manager'))) {
			...
			$columnName =  'maKhoiLop'; // Column name
			if (is_array($_POST['columns'][$columnIndex]['data'])) {
				$columnName = empty($_POST['columns'][$columnIndex]['data']['sort']) ? 'maKhoiLop' : htmlspecialchars($_POST['columns'][$columnIndex]['data']['sort']);
			} else {
				$columnName = empty($_POST['columns'][$columnIndex]['data']) ? 'maKhoiLop' : htmlspecialchars($_POST['columns'][$columnIndex]['data']);
			}
			...
			$searchQuery = " ";
			if($searchValue != ''){
			   $searchQuery .= " AND (`tenKhoiLop` like '%".$searchValue."%') ";
			}
			...
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `khoilop`");
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecords = $records['allcount'];
			 
			## Total number of records with filtering
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `khoilop` WHERE 1 ".$searchQuery);
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecordwithFilter = $records['allcount'];
			 
			## Fetch records
			$empQuery = "SELECT * FROM `khoilop` WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
			//echo $empQuery;
