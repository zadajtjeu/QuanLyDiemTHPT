...
if((empty($_SESSION['username']) && empty($_SESSION['password']))) {
	...
	if ($result->num_rows > 0) {
		...
		if (in_array($taikhoan['role'], array('admin', 'manager'))) {
			...
			$columnName =  'maHK'; // Column name
			if (is_array($_POST['columns'][$columnIndex]['data'])) {
				$columnName = empty($_POST['columns'][$columnIndex]['data']['sort']) ? 'maHK' : htmlspecialchars($_POST['columns'][$columnIndex]['data']['sort']);
			} else {
				$columnName = empty($_POST['columns'][$columnIndex]['data']) ? 'maHK' : htmlspecialchars($_POST['columns'][$columnIndex]['data']);
			}
			...
			$searchQuery = " ";
			if($searchValue != ''){
			   $searchQuery .= " AND (`tenHK` like '%".$searchValue."%') ";
			}
			...
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `hocky`");
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecords = $records['allcount'];
			 
			## Total number of records with filtering
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `hocky` WHERE 1 ".$searchQuery);
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecordwithFilter = $records['allcount'];
			 
			## Fetch records
			$empQuery = "SELECT * FROM `hocky` WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
			//echo $empQuery;
