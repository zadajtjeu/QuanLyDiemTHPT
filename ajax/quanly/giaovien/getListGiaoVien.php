...
if((empty($_SESSION['username']) && empty($_SESSION['password']))) {
	...
	if ($result->num_rows > 0) {
		...
		if (in_array($taikhoan['role'], array('admin', 'manager'))) {
			...
			$columnName =  'maGV'; // Column name
			if (is_array($_POST['columns'][$columnIndex]['data'])) {
				$columnName = empty($_POST['columns'][$columnIndex]['data']['sort']) ? 'maGV' : htmlspecialchars($_POST['columns'][$columnIndex]['data']['sort']);
			} else {
				$columnName = empty($_POST['columns'][$columnIndex]['data']) ? 'maGV' : htmlspecialchars($_POST['columns'][$columnIndex]['data']);
			}
			...
			$searchQuery = " ";
			if($searchValue != ''){
			   $searchQuery .= " AND (`tenGV` like '%".$searchValue."%' OR
			            `ngaySinh` LIKE '%".$searchValue."%' OR `diaChi` LIKE '%".$searchValue."%') ";
			}
			...
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `giaovien`");
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecords = $records['allcount'];
			 
			## Total number of records with filtering
			$sel = $mysqli->query("SELECT count(*) AS `allcount` FROM `giaovien` WHERE 1 ".$searchQuery);
			$records = $sel->fetch_array(MYSQLI_ASSOC);
			$totalRecordwithFilter = $records['allcount'];
			 
			## Fetch records
			$empQuery = "SELECT * FROM `giaovien` WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
			//echo $empQuery;
