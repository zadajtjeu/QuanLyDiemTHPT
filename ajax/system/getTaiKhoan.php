...
if((empty($_SESSION['username']) && empty($_SESSION['password']))) {
	...
	if ($result->num_rows > 0) {
		...
		if (in_array($taikhoan['role'], array('admin', 'manager'))) {
			...
			$columnName = empty($_POST['columns'][$columnIndex]['data']) ? 'id' : htmlspecialchars($_POST['columns'][$columnIndex]['data']); // Column name
			...
			## Fetch records
			$empQuery = "SELECT * FROM `taikhoan` WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
			//echo $empQuery;
